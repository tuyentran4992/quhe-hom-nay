...
| `gate-result` | đã unlock + OK | vùng bài luận: dòng `gate-result-question` (nếu có) + thân bài `{br}` → `<br>`, nút `gate-ask` "Xin luận sâu" còn dùng. **ANIM-LUAN (t_74502491)**: cả bài done LẪN bài saved (bấm `gate-review`) mang class `luan-fade` — fade opacity 0→1 + translateY(6px→0), 280ms ease-out, tắt hẳn khi `prefers-reduced-motion: reduce`. QA visual: article phải MỎT tay hiện, không nhảy cụt |
| `gate-submit-spinner` | `phase submitting` — từ lúc bấm `gate-ask`/`gate-retry` cho tới khi POST #5 trả lời (ANIM-LUAN t_74502491) | vòng spinner CSS thuần (border + @keyframes `gate-spin`, màu `currentColor` kế thừa nhãn nút — không lib, không màu mới) nằm TRONG nút gate-ask. QA bấm mạng chậm: phải thấy spinner + nút disabled + nhãn "Đang xin luận…" trong ≤100ms đầu; ô hỏi `gate-question` + chip + counter GIỮ NGUYÊN trong DOM (không layout jump). 429/409/500 → spinner biến mất, sang branch cooldown/cap/saved/failed như cũ |
